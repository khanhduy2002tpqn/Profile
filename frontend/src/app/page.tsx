    ...
    <div className="relative min-h-screen bg-slate-950 text-slate-100 flex flex-col justify-between overflow-hidden">
      ...
      <header className="w-full px-6 py-4 flex justify-between items-center border-b border-slate-900 bg-slate-950/50 backdrop-blur-md z-10">
        ...
        <button
          ...
          className="text-xs uppercase tracking-wider font-semibold text-slate-400 hover:text-slate-200 border border-slate-800 hover:border-slate-700 px-4 py-2 rounded-full transition-all duration-300 cursor-pointer"
        >
          Admin Portal
        </button>
      </header>
      ...
      <main className="flex-1 flex flex-col justify-center items-center px-4 relative z-10">
        <div className="text-center max-w-xl mb-8 space-y-4">
          <div className="inline-flex items-center space-x-2 bg-violet-950/30 border border-violet-900/50 px-3 py-1 rounded-full text-violet-400 text-xs font-semibold uppercase tracking-wider">
          ...
          <p className="text-slate-400 text-sm md:text-base leading-relaxed">
            Nhập mã CampID cá nhân của bạn (ví dụ: <code className="text-slate-200 bg-slate-900 px-2 py-0.5 rounded font-mono">SC2026-CAMP001</code>) để tra cứu album hoạt động, chứng nhận và các dự án của bạn.
          </p>
        </div>
        ...
        <div className="w-full max-w-lg p-6 bg-slate-900/40 border border-slate-850 rounded-2xl backdrop-blur-xl shadow-2xl relative">
          <form onSubmit={handleSearch} className="space-y-4">
            <div className="relative">
              <input
                ...
                disabled={loading}
                className="w-full pl-12 pr-4 py-4 rounded-xl bg-slate-950/60 border border-slate-800 focus:border-violet-500 focus:outline-none focus:ring-1 focus:ring-violet-500 transition-all duration-300 placeholder-slate-500 text-slate-100 uppercase tracking-widest text-center font-bold font-mono"
              />
          ...
          {/* Validation Feedback */}
          {error && (
            <div className="mt-4 p-3 bg-rose-950/20 border border-rose-900/50 text-rose-400 rounded-xl flex items-center space-x-2 text-xs md:text-sm animate-bounce">
